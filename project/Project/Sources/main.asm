...
; Made by: Jason Su, Kyle Chin, Evan Luu
; *****************************************************************
...
; variable section for Guider and sensors
;*******************************************************************
              ORG   $3800

; Initial sensor values based on the initial readings & variance
; -------------------------------------------------------
BASE_LINE     FCB   $9D
BASE_BOW      FCB   $CA
BASE_MID      FCB   $CA
...
;***************************************************************************************************
;From Lab 5
DISPATCHER        JSR   NOT_START                        ; Dispatcher Start
                  RTS

NOT_START      CMPA  #START                                ; Verify if state is START
                  BNE   NOT_FORWARD                      ; If not, move to FORWARD state validation
                  JSR   START_ST                              ; Validate START state
                  RTS                                         

NOT_FORWARD    CMPA  #FWD                                  ; Verify if state is FORWARD
                  BNE   NOT_STOP                         ; If not, move to ALL_STOP state validation
                  JSR   FWD_ST                                ; Validate FORWARD state
                  RTS
                  
NOT_REV_TRN    CMPA  #REV_TRN                              ; Verify if state is REV_TURN
                  BNE   NOT_LEFT_ALIGN                   ; If not, move to LEFT_ALIGN state validation
                  JSR   REV_TRN_ST                            ; Validate REV_TURN state
                  RTS                                           

NOT_REVERSE       CMPA #ALL_STP ; Else if it’s the ALL_STOP state                   
                  BNE NOT_STOP  ;                                               
                  JSR ALL_STOP_ST ; then call the ALL_STOP routine                   
                  RTS ; and exit                                          

NOT_STOP       CMPA  #ALL_STP                             ; Verify if state is ALL_STOP
                  BNE   NOT_LEFT_TRN                     ; If not, move to LEFT_TURN state validation
                  JSR   ALL_STOP_ST                           ; Validate ALL_STOP state
                  RTS                                         

;Turn and Allign States
NOT_LEFT_TRN   CMPA  #LEFT_TRN                             ; Verify if state is LEFT_TURN
                  BNE   NOT_RIGHT_TRN                    ; If not, move to RIGHT_TURN state validation
                  JSR   LEFT                                  ; Validate LEFT_TURN state
                  RTS                                                                                                                      

NOT_LEFT_ALIGN CMPA  #LEFT_ALIGN                         ; Verify if state is LEFT_ALIGN
                  BNE   NOT_RIGHT_ALIGN                ; If not, move to RIGHT_ALIGN state validation
                  JSR   LEFT_ALIGN_DONE                     ; Validate LEFT_ALIGN state
                  RTS

NOT_RIGHT_TRN  CMPA  #RIGHT_TRN                          ; Verify if state is RIGHT_TURN
                  BNE   NOT_REV_TRN                    ; If not, move to REV_TURN state validation
                  JSR   RIGHT                               ; Validate RIGHT_TURN state                                      

NOT_RIGHT_ALIGN CMPA  #RIGHT_ALIGN                       ; Verify if state is RIGHT_ALIGN
...
;***************************************************************************************************
;To make small Left turn adjustments when off path
...
;*************************************************************************************************** 
;To make small right turn adjustments when off path
...
REV_TRN_ST        LDAA    SENSOR_BOW        ;load bow sensor                                                      
                  ADDA    BOW_VARIANCE                                                                 
                  CMPA    BASE_BOW                                                                
                  BMI     EXIT                                                                    
                  JSR     INIT_LEFT                                                               
